...
**Đã làm**:
- **Map Editor**: nhiều layer terrain (thêm/xoá/đổi tên/ẩn-hiện/sắp xếp thứ tự chồng lớp, chọn layer đang vẽ), import tileset ảnh bất kỳ (hỗ trợ margin/spacing kiểu Tiled, cắt lưới pixel-perfect qua canvas — không lệch/nhoè), chọn 1 hoặc nhiều ô cùng lúc để vẽ theo cụm, khung xem trước đúng kích thước cọ vẽ/tẩy khi rê chuột trên canvas, tô hết cả layer bằng 1 nút, gắn animation nhiều frame cho từng tile bằng kéo bôi đen (tự chạy trên canvas), đặt 6 loại building + 4 loại unit lên lưới (chạm/click để đặt, kéo để di chuyển, chỉnh property), vẽ khu vực Spawn/Trigger/Boundary, zoom/pinch-zoom + pan, resize map, save/load JSON (tự chứa cả ảnh tileset, tương thích ngược file cũ chưa có layer).
